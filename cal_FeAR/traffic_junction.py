...
class TrafficJunction(gym.Env):
    ...
    # ***************************** modified**********************************
    def cal_FeAR_ij(agent_i, agent_j, agents_action):
        '''
        Function that calculates the FeAR value of agent i on agent j, i.e. FeAR_ij

        agent_i: the id of agent i, int
        agents_action: list of actions of all the agents to perform in the environment, list
        ...
        def is_neighbors(location_i, location_j):
            '''
            Function that decides whether agent i and agent j is neighbor
            return: bool
            '''

            r = 2
            if np.norm(location_i - location_j) < r:
                return True
            else:
                return False # if distance between i and j > r, then i cannot affect j by any actions in the action space


        def count_FeasibleActions(movement_i, location_i, location_j, MdR_j):
            '''
            Function that counts the number of feasible actions of agent j given the movement of agent i

            movement_i: the potential movement of agent i, np_array, shape=(2,)
            location_i: the current location of agent i, np_array, shape=(2,)

            return: the number of feasible actions of agent j given the movement of agent i, int
            '''

            new_location_i = movement_i + location_i

            for feasible_location_j in [location_j, location_j + MdR_j]:
                if np.norm(new_location_i - feasible_location_j) < epsilon:
                    return 1
            else:
                return 2


        # current locations of agents before actions
            location_i = np.array(self.agent_pos[agent_i])
            location_j = np.array(self.agent_pos[agent_j])

        if agent_i == agent_j:
            return 0.0 # we only use the FeAR values when i != j for training

        elif is_neighbors(location_i, location_j) == False:
            return 0.0 # if agent i and j are far away, we assume agent i has no impact on j

        else:
            # we assume that the actions are checked by the envirment such that they are valid, 
            # i.e. agents will not be out of boundary after the actions
            action_i = np.array(agents_action[agent_i])
            action_j = np.array(agents_action[agent_j])
            # by definition the MdRs should also be valid
            MdR_i = np.array(self._agents_direction[agent_i])
            MdR_j = np.array(self._agents_direction[agent_j])
           
            n_FeasibleActions_MdRi_j = count_FeasibleActions(MdR_i, location_i, location_j, MdR_j)
            n_FeasibleActions_Actioni_j = count_FeasibleActions(action_i, location_i, location_j, MdR_j)
            FeAR_ij = (n_FeasibleActions_MdRi_j - n_FeasibleActions_Actioni_j) / (n_FeasibleActions_MdRi_j + epsilon)
    ...
    def step(self, agents_action):
        ...
        for agent_i, action in enumerate(agents_action):
            if not self._agent_dones[agent_i] and self._on_the_road[agent_i]:
                ...
                # ***************************** modified **********************************
                # calculate FeAR_ij and add the sum of FeAR values for all j into reward_i
                FeAR_ij = 0.0
                weight_FeAR = -10.0
                for agent_j in range(self.n_agents):
                    FeAR_ij += cal_FeAR_ij(agent_i, agent_j, agents_action)
                rewards[agent_i] += weight_FeAR * FeAR_ij
                # ***************************** modified **********************************
